...
A4 = imclose(i,se);    %   闭操作
figure(2);
subplot(231),imshow(i),title('二值图像');
subplot(232),imshow(se.Neighborhood),title('结构元');
subplot(233),imshow(A1),title('膨胀后的二值图像');
subplot(234),imshow(A2),title('腐蚀后的二值图像');
subplot(235),imshow(A3),title('开运算后的二值图像');
subplot(236),imshow(A4),title('闭运算后的二值图像');

A5 =i-imerode(i,se);  %边界提取：原图减去腐蚀后的图
% 图像填充
B0 = zeros(o,j);
B0(50, 150) = 1;
B1=imdilate(B0, se) & ~i;
...
subplot(232),imshow(se.Neighborhood),title('结构元');
subplot(233),imshow(H1),title('膨胀后的灰度图像');
subplot(234),imshow(H2),title('腐蚀后的灰度图像');
...
subplot(222),imshow(ones(14,1)),title('结构元');
subplot(223),imshow(mark),title('标记图像');
